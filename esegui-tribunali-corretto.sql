...
-- Versione corretta senza ON CONFLICT
...
-- STEP 2: Inserisci i tribunali di Roma e Milano (solo se non esistono già)
INSERT INTO tribunali (name, city, address, type) 
SELECT * FROM (VALUES 
...
) AS new_tribunali(name, city, address, type)
WHERE NOT EXISTS (
    SELECT 1 FROM tribunali t 
    WHERE t.name = new_tribunali.name 
    AND t.city = new_tribunali.city 
    AND t.type = new_tribunali.type
);
